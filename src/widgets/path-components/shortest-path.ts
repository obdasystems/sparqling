...
/** @internal */
export default class ShortestPathDialog extends ui.GscapeConfirmDialog {

  public classes?: ui.EntityViewData[]
  ...
  constructor(grapholscape?: Grapholscape) {
    super()
    if (grapholscape)
      this.classes = ui.createEntitiesList(grapholscape, { class: 1, areAllFiltersDisabled: false } )
  }
  ...
  closePanel(): void {
    ...
    entitySelectors?.forEach(entitySelector => (entitySelector as ui.GscapeEntitySelector).closePanel())
    this.requestUpdate()
  }
  ...
  openPanel(): void {
    ...
    entitySelectors?.forEach(entitySelector => (entitySelector as ui.GscapeEntitySelector).openPanel())
    this.requestUpdate()
  }

  public onConfirm(callback: (sourceClassIri: string, targetClassIri: string) => void): ui.GscapeConfirmDialog {
          ...
          <div class="gscape-panel selected-entity-wrapper">
            ...
            ${allowClear 
              ? html`
                ...
                >
                  ${ui.getIconSlot('icon', ui.icons.cross)}
                </gscape-button>
              `
              : null
            }
...
customElements.define('sparqling-shortest-path-dialog', ShortestPathDialog)